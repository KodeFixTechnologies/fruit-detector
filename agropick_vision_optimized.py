...
Optimized version - better serial sync with ESP32
"""
...
SERVO_DELAY = 0.15  # Reduced from 0.3 - ESP32 moves are non-blocking now
TWIST_ANGLE = 15
TWIST_CYCLES = 3
TWIST_DELAY = 0.20
APPROACH_H = 8.0
...
class SerialManager:
    ...
    def connect(self) -> bool:
        try:
            self.ser = serial.Serial(ESP32_PORT, ESP32_BAUD, timeout=0.1)
            time.sleep(2)
    ...
    def send(self, cmd: str, wait_ack: bool = True) -> str | None:
        """Send command and optionally wait for ACK line starting with OK: or ERR:"""
        with self._lock:
            ...
            try:
                ...
                while self.ser.in_waiting:
                    line = self.ser.readline().decode("utf-8", errors="ignore").strip()
                    if line and not line.startswith("S:"):  # ignore status lines
                        log(f"ESP32: {line}")
                ...
                while time.time() < deadline:
                    if self.ser.in_waiting:
                        ...
                        if line.startswith("OK:") or line.startswith("ERR:"):
                            return line
                        if line == "BUSY" or line == "READY":
                            return line
                        if line.startswith("POS:") or line.startswith("DIST:"):
                            return line
                        # Status line or other output, log and continue waiting
                        if not line.startswith("S:"):
                            log(f"ESP32: {line}")
                    else:
                        time.sleep(0.005)

                log(f"No ACK for: {cmd}", "WARN")
                return None
    ...
    def wait_ready(self, timeout: float = 10.0) -> bool:
        """Poll ESP32 until arm reports READY."""
        deadline = time.time() + timeout
        while time.time() < deadline:
            resp = self.send("busy", wait_ack=True)
            if resp == "READY":
                return True
            time.sleep(0.1)
        log("Timeout waiting for arm ready", "WARN")
        return False
...
class ArmController:
    ...
    def _send(self, servo: str, angle: int) -> None:
        ...
        resp = serial_mgr.send(f"{servo}:{angle}", wait_ack=True)
        if resp and resp.startswith("OK:"):
            self.positions[servo] = angle
        elif resp and resp.startswith("ERR:"):
            log(f"Servo error: {resp}", "ERROR")
        # Small delay for the servo to physically start moving
        time.sleep(0.05)

    def _wait_arm(self) -> None:
        """Wait until ESP32 reports arm is no longer moving."""
        serial_mgr.wait_ready(timeout=8.0)
    ...
    def move_to_xyz(self, x: float, y: float, z: float) -> None:
        ...
        # Wait for all moves to complete on ESP32
        self._wait_arm()

    def home(self) -> None:
        log("Moving to HOME", "ARM")
        self.busy = True
        resp = serial_mgr.send("home", wait_ack=True)
        if resp:
            log(f"Home started: {resp}", "ARM")
        # Wait for home sequence to complete
    ...
    def pick(self, x: float, y: float, diameter_cm: float = 5.0) -> bool:
        ...
        self.busy = True
        try:
            log("1/6: Approach high", "ARM")
            self.move_to_xyz(x, y, GRAB_H + APPROACH_H)

            log("2/6: Open gripper", "ARM")
            self.open_gripper()

            log("3/6: Lower to target", "ARM")
    ...
    def run_pipeline(self, image_bytes: bytes) -> None:
        ...
        def execute_all() -> None:
            for index, call in enumerate(calls, start=1):
                ...
                # No extra sleep needed - commands now wait for completion
            self.picks_successful += 1
            log(
...
# Draw UI (unchanged from original)
def draw_ui(frame: Any, auto_mode: bool) -> Any:
    ...
    cv2.putText(out, f"ARM:{'BUSY' if arm.busy else 'READY'}", (90, 27),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, arm_color, 2)
    cv2.putText(out, f"AUTO:{'ON' if auto_mode else 'OFF'}", (230, 27),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, auto_color, 2)
    cv2.putText(out, f"Picks:{gemini.picks_successful}/{gemini.picks_attempted}",
                (360, 27), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

    if gemini.scene_desc:
        cv2.rectangle(out, (0, 40), (width, 62), (10, 10, 40), cv2.FILLED)
        cv2.putText(out, gemini.scene_desc[:80], (10, 57),
    ...
    cv2.putText(out, "LOG", (width - panel_width + 8, 82),
    ...
    for index, line in enumerate(lines):
        ...
        if y_pos > height - 60:
            break
        color = (200, 200, 200)
        if "[SUCCESS]" in line: color = (0, 255, 100)
        elif "[ERROR]" in line: color = (0, 80, 255)
        elif "[WARN]" in line: color = (0, 165, 255)
        elif "[ARM]" in line: color = (255, 200, 0)
        elif "[GEMINI]" in line: color = (200, 100, 255)
        elif "[PICK]" in line: color = (0, 255, 200)
        cv2.putText(out, line[:38], (width - panel_width + 5, y_pos),
    ...
    cv2.putText(out, controls, (8, height - 10),
...
def main() -> None:
    print(
        """
+------------------------------------------------------+
| AgroPick - Gemini Vision + Arm (Optimized)           |
+------------------------------------------------------+
